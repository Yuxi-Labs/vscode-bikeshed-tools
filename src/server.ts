...
/* ──────────────────────────────────────────────────── */
/*  Static (placeholder) completion & hover tables      */
/*  – hydrate these from Bikeshed data at start-up      */
/* ──────────────────────────────────────────────────── */
const bikeshedMetaKeys = [
  'Title', 'Shortname', 'Level', 'Status', 'ED',
  'TR', 'Latest', 'Previous', 'Editor', 'Abstract'
] as const;
...
connection.onCompletion(params => {
  ...
  /* very basic context check – refine later */
...
connection.onHover(params => {
  ...
  const entry = hoverDocs[word.toLowerCase()];
  if (!entry) return null;

  const hover: Hover = {
    contents: { kind: 'markdown', value: entry }
  };
  return hover;
});
